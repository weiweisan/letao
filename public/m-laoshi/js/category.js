$(function () {
    ...
    $.ajax({
        ...
        url:'/category/queryTopCategory',// url 请求地址 也必须要 注意带/
        // dataType:'json',// 默认也是 把json转出JS对象 可以省略
    ...
    $('.category-left ul').on('tap','li > a',function (){
        ...
        // console.log(this.dataset['id']);// 拿了之后不会做类型转换
        // 2. 获取到了当前点击a的分类的id
        var id = $(this).data('id');
        // 3. 根据当前id请求二级分类的API数据  调用获取右侧分类的函数传入id
        querySecondCategory(id);     
        // 4. 给当前点击a的父元素添加active 其他兄弟删除active  siblings 兄弟元素  是a的父元素 li的所有兄弟
        $(this).parent().addClass('active').siblings().removeClass('active');   
    });
    // 5. 为了默认也去调用请求右侧分类的数据 并且显示第一个分类的数据 传人id 为 1 
    ...
    // zepto的 get必须挨个儿传参 不能是对象 不推荐使用
    // $.get('/category/queryTopCategory',{'id':1},function (data){
    //     console.log(data);
